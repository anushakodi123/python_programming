...
def get_frequencies(input_iterable):
    ...
    for i in input_iterable:
        if i in count_hash:
            count_hash[i] = count_hash[i] + 1
        else:
            count_hash[i] = count + 1
...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    all_keys = list(set(dict1_keys + dict2_keys))
    ...
    return  1-(sum_of_diff/sum_of_sum)
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    """
    keys1 = freq_dict1.keys()
    keys2 = freq_dict2.keys()
    all_keys = list(keys1) + list(keys2)
    new_dic = {}
    count = 0
    for i in all_keys:
        count = freq_dict1[i] + freq_dict2[i]
        new_dic[i] = count
    max_value = max(new_dic.values())
    max_keys = sorted([k for k, v in new_dic.items() if v == max_value])
    return max_keys
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for k,v in word_dic.items():
        final_dic[k] = math.log10(len(file_paths)/v)
    return final_dic
...
def get_tfidf(tf_file_path, idf_file_paths):
    """
        ...
        * TF-IDF(i) = TF(i) * IDF(i)
        """
    result_dic = {}
    tf_value = get_tf(tf_file_path)
    idf_value = get_idf(idf_file_paths)
    for k,v in tf_value.items():
        result_dic[k] = v * idf_value[k]
    return result_dic
